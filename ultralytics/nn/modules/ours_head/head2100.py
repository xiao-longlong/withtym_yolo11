...
class Detect2100(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        # self.cv2_3 = nn.ModuleList(nn.Conv2d(c2, 4 * self.reg_max, 1) for x in ch )
        ...
        self.ClsAttentionLayers = nn.ModuleList(
                nn.Sequential(
                    *[
                        ...
                        Conv(
                            int(2 * 4 * 256 * 0.5),
                            int((2 * 4 * 256 * 0.5 + self.num_classes * self.num_classes)/2),
                            1,
                            1
                        ...
                        Conv(
                            int((2 * 4 * 256 * 0.5 + self.num_classes * self.num_classes)/2),
                            int(self.num_classes * self.num_classes),
                            1,
                            1 
                        ),
                    ]
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            cls_x = x
            cls_feat_1 = self.cls_convs_2101_1[i](cls_x)
            cls_feat_2 = self.cls_convs_2101_2[i](cls_feat_1)
            cls_feat_3 = self.cls_convs_2101_3[i](cls_feat_2)
            cls_feat_up = torch.concat([cls_x, cls_feat_1, cls_feat_2, cls_feat_3], 1)
            ourcls_feat = self.ClsAttentionLayers[i](cls_feat_up)
            ...
            clsoutput_ori = self.cv2[i](x).permute(0, 2, 3, 1).unsqueeze(-1)
            cls_output = torch.einsum('ijklm,ijkmn->ijkln', ourcls_atten, clsoutput_ori).squeeze(-1).permute(0, 3, 1, 2)
            x[i] = torch.cat((cls_output, self.cv3[i](x[i])), 1)
            # x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
        if self.training:  # Training path
            return x
        y = self._inference(x)
